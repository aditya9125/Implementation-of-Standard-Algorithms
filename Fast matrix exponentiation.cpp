...
void makeIdentityMat(VVI &I,int n)		///makes identity matrix of nxn
{
	I= VVI(n+1,VI(n+1));
	for(int i=0;i<n;i++)
	 for(int j=0;j<n;j++)
...
void MatExpo(VVI A,ll expo,int n,VVI &ANS)	///to obtain A^n (where A is a square matrix)	,ANS would be the answer matrix
{
	//identity matrix
...
int main()
{
	...
	VVI A=VVI(n,VI(n));//,B=VVI(b+1,VI(c+1));
