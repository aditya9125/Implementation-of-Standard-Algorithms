...
template<typename type>
void show_arr(type a[],type n=1)
{
    for(type i=0; i<n; i++) cout<<a[i]<<" ";
}
...
template<typename type>
inline type min4(type a,type b,type c,type d)
{
    return min(a,min3(b,c,d));
}

template<typename type>
inline type max4(type a,type b,type c,type d)
{
...
template<typename type>
type modpowIter(type a, type b, type c)
{
    type ans=1;
    while(b != 0)
    {
...
template<typename type>  unsigned long long fact(type x)
{
    ull ans=1;
    for(type i=1; i<=x; i++)
    {
        ans*=i;        /*ans%=MOD;   /Optional*/
    }
    return ans;
}
...
{
    type ans=0,i;
    rep(i,0,s.size())	ans=ans*10+(s[i]-'0');
    return ans;
}

template<typename T_> inline bool is_perfsq(T_ n)
{
    T_ a=(T_)sqrt((double) n);
    return (a*a==n || (a+1)*(a+1)==n);/* in case of an off-by-one float error*/
}
...
int main()
{
    ...
    //	#ifndef ONLINE_JUDGE ifstream cin("/home/aditya/Documents/ip"); #endif

    //ofstream cout("/home/aditya/Documents/op");

    while(true)
    {
        ll n;	///no of rectangles in the histogram
        scanll(n);
        ...
        /// finding L for each bar(rectangle);
        stack<ll> st;
        ...
        for(ll i=1; i<=n; i++)
        {
            ...
            if(!st.empty() and ht[st.top()]<ht[i])
            {
                L[i]=i-st.top();
                st.push(i);
            }
        ...
        ///Finding R for each bar
        st= stack<ll>();

        //cout<<"size of new st ="<<st.size()<<nl;
