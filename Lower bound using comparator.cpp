...
class Data
{
	...
	public:
		...
		Data(int x,int y,int z){ val= x	;d= y,cost= z};
};
*/

bool cmp( pair<int,II> OB,int val)
{
	return (OB.F < val);
}
...
class Comparator
{
	...
	public :
		...
		bool operator()(pair<int,II> OB1,pair<int,II> OB2)
		{
			if(OB1.F == OB2.F)
			{
				if(OB1.S.F==OB2.S.F)
				{
					return (OB1.S.S<OB2.S.S);



				}
				return (OB1.S.F<OB2.S.F);
			}
			return OB1.F<OB2.F;
		}

/*
        bool operator() ( pair<int,II> OB,int val )
        {
			return (OB.F < val);
		}
*/
...
int main()
{
	...
	sort(whole(v),Comparator()); ///passing an object of the Comparator class .
	//also we could have created one and then passed into sort as,
	//Comparator obj;
	//sort(whole(v),obj)

	for(int i=0;i<v.size();i++)
		cout<<v[i].F<<' '<<v[i].S.F<<" "<<v[i].S.S<<nl;
